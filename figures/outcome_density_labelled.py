"""Figure: outcome density vs initialisation, R = (0, 0.8, 1.0) (high-drift RLOO).

Reward-relabelled variant of ``outcome_density.py`` for the rewritten section:
a1 = task failure (R1 = 0), a2 = faithful completion (R2 = 0.8), a3 = hacking
(R3 = 1.0). Four panels side by side, all starting at pi_1 = 0.9 but leaning
progressively away from hacking: pi_2 in {0.05, 0.06, 0.07, 0.08}
(pi_3 = 0.1 - pi_2). Each panel shows, at G = 8, eta = 1.2, steps = 25:
...
* below, a density of the final policy's x-position (x = 0 at faithful a2,
  x = 1 at hacking a3), with the deterministic endpoint marked in blue.
...
Run: ``uv run python figures/outcome_density_labelled.py``
  -> figures/out/outcome-density-1.0.svg
"""
...
def build() -> str:
    ...
    svg.text(LEFT, BASELINE + 13, "R₂ = 0.8", fill=muted, font_size=9, text_anchor="start")
    svg.text(LEFT + SCALE, BASELINE + 13, "R₃ = 1.0", fill=muted, font_size=9,
             text_anchor="end")
...
def main() -> None:
    ...
    dest = OUT / "outcome-density-1.0.svg"
